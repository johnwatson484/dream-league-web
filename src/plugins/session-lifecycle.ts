...
const plugin: Plugin<ServerOptions> = {
  ...
  register: (server) => {
    server.ext('onPreAuth', async (request, h) => {
      ...
      const sessionCookie = (request.state as Record<string, { sessionId?: string } | undefined>)?.[cookieName]
      ...
      if (session.accessTokenExpiresAt < Date.now() + bufferMs) {
        try {
          const response = await post('/token/refresh', { refreshToken: session.refreshToken }) as { accessToken: string; refreshToken: string }
